# -*- coding: utf-8 -*-
"""
Created on Tue Dec 13 02:43:46 2016
...
XGB_learning_rate = np.arange(0.1,1,0.1)# : float, optional (default=0.1)
XGB_max_depth = np.arange(2,30,3)
XGB_subsample = np.arange(0.1,1,0.3)# float, optional (default=1.0)

###########################################################################
Count = 1
Error = pd.DataFrame(columns=('learning rate ','max depth ','subsample ','ERROR'))

for i in range(3):
    for rate in XGB_learning_rate:
        for maxd in XGB_max_depth:
...
#                est = SVR(gamma=gamma,C=error,epsilon = eps)
                ...
                num_round = 2
                model = xgb.XGBRegressor(max_depth = maxd, learning_rate=rate, subsample=subS, objective='reg:linear')
                model.fit(train, Sp_train)
                # make prediction
...
#               print(Res.tail())
                ...
                Error.loc[Count] = [rate, maxd, subS,(np.round(np.sqrt(np.mean(Res.Error)),3))]
                Count += 1
            print("")
            print(Error.tail(8))
Error.to_csv("Error_XGB_RF.csv", sep=',', encoding='utf-8')
print("\n\n\n\n\nRF done")
###########################################################################