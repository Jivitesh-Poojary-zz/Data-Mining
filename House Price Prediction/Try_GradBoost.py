...
@author: Shreyas Rewagad and Jivitesh Poojary
"""
...
HouseTrain = pd.read_csv("/nfs/nfs4/home/srewagad/HousePrice/train.csv", index_col = 0, parse_dates=True, header = 0)
...
HouseTest = pd.read_csv("/nfs/nfs4/home/srewagad/HousePrice/test.csv", index_col = 0, parse_dates=True, header = 0)
...
GB_learning_rate = np.arange(0.1,1,0.3)# : float, optional (default=0.1)
GB_n_estimators = np.arange(1000,2000,500)
GB_subsample = np.arange(0.1,1,0.3)# float, optional (default=1.0)

###########################################################################
Count = 1
Error = pd.DataFrame(columns=('loss ','learning_rate ','n_estimators ','subsample','ERROR'))

for i in range(3):
    for learn in GB_learning_rate:
        for NEstimator in GB_n_estimators:
            for subS in GB_subsample:
                for Loss in GB_loss:
                    ...
                    est = GradientBoostingRegressor(loss = Loss,learning_rate=learn,n_estimators=NEstimator,subsample =subS)
...
#                    print(Res.tail())
                    ...
                    Error.loc[Count] = [Loss,learn,NEstimator,subS,(np.round(np.sqrt(np.mean(Res.Error)),3))]
                    Count += 1
                print("")
                print(Error.tail(8))
Error.to_csv("Error_GradiantBoost_RF.csv", sep=',', encoding='utf-8')
print("\n\n\n\n\nRF done")
###########################################################################